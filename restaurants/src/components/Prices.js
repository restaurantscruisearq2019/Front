...
class Prices extends React.Component {
  ...
  fetchRestaurants = ( a ) => {
    apollo
      ...
      .then(data => {
        this.setState({ priceRangeRestaurants: data.data.getRestaurantsByPricerange });
  ...
  renderRestaurants(){
    ...
    return this.state.priceRangeRestaurants.map( r => {
      return (
        <CategoryCard key={r._id} name={r.name} category={this.state.priceRange}/>
      )
    });
  }
        ...
        className="ui sixteen wide column two column relaxed stackable grid middle aligned content container">
          <div className="four wide column">
            ...
            <div className="ui secondary vertical pointing menu">
              ...
              <button onClick={() => this.fetchRestaurants("20-30")} className="item">
                20-30 (Dollars)
              </button>
